...
function card(p,i){{
  ...
  return "<div class=\\"pcard\\" onclick=\\"sel("+i+")\\">"
    ...
    +"<div style=\\\"height:5px;background:#f0f0ec;border-radius:3px;overflow:hidden;display:flex;margin-bottom:2px;\\\">"+"<div style=\\\"width:"+(Math.min(p.stats.log_h/160*100,100)).toFixed(1)+"%;background:"+(p.stats.log_h>p.stats.est_h&&p.stats.est_h>0?"#e24b4a":"#1d9e75")+";\\\"></div>"+"<div style=\\\"width:"+(Math.min(Math.max(0,p.stats.est_h-p.stats.log_h)/160*100,100)).toFixed(1)+"%;background:#f0a500;opacity:.7;\\\\"></div>"+"</div>"+"<div style=\\\"display:flex;justify-content:space-between;font-size:8.5px;color:#aaa;margin-bottom:5px;\\\">"+"<span style=\\\"color:#1d9e75;\\\">&#9632; "+p.stats.log_h.toFixed(1)+"h log</span>"+"<span style=\\\"color:#f0a500;\\\">&#9632; "+Math.max(0,p.stats.est_h-p.stats.log_h).toFixed(1)+"h rest</span>"+"<span>/ 160h</span>"+"</div>"
