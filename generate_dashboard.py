...
function card(p,i){{
  ...
  var lh=p.stats.log_h,rh=Math.max(0,p.stats.est_h-lh);
  var lp=Math.min(lh/160*100,100).toFixed(1);
  var rp=Math.min(rh/160*100,100).toFixed(1);
  var over=lh>p.stats.est_h&&p.stats.est_h>0;
  var logColor=over?"#e24b4a":"#1d9e75";
  var capex=p.stats.capex_pct,opex=p.stats.opex_pct;
  var html="<div class=\"pcard\" onclick=\"sel("+i+")\">";
  ...
  html+="<div style=\"display:flex;justify-content:space-between;font-size:9.5px;color:#999;margin-bottom:4px;\">";
  html+="<span>"+p.stats.total+" issues &middot; "+p.stats.est_h.toFixed(1)+"h</span>";
  html+="<span>"+pct.toFixed(0)+"% cap</span></div>";
  html+="<div style=\"height:5px;background:#f0f0ec;border-radius:3px;overflow:hidden;display:flex;margin-bottom:2px;\">";
  html+="<div style=\"width:"+lp+"%;background:"+logColor+";\"></div>";
  html+="<div style=\"width:"+rp+"%;background:#f0a500;opacity:.7;\"></div>";
  html+="</div>";
  html+="<div style=\"display:flex;justify-content:space-between;font-size:8.5px;color:#aaa;margin-bottom:4px;\">";
  html+="<span style=\"color:"+logColor+"\">&#9632; "+lh.toFixed(1)+"h log</span>";
  html+="<span style=\"color:#f0a500;\">&#9632; "+rh.toFixed(1)+"h rest</span>";
  html+="<span>/ 160h</span></div>";
  if(capex+opex>0){{
    html+="<div style=\"height:3px;display:flex;border-radius:2px;overflow:hidden;margin-bottom:2px;\">";
    html+="<div style=\"flex:"+capex+";background:#378add;\"></div>";
    html+="<div style=\"flex:"+opex+";background:#f0a500;\"></div></div>";
    html+="<div style=\"font-size:8.5px;color:#aaa;margin-bottom:4px;\">";
    html+="<span style=\"color:#378add;font-weight:600;\">&#9632;CAP "+capex+"%</span> ";
    html+="<span style=\"color:#f0a500;font-weight:600;\">&#9632;OPE "+opex+"%</span></div>";
  }}
  html+="<div class=\"divider\"></div>";
  if(hR) html+="<div class=\"board-row\"><span class=\"board-label lreq\">REQ</span><div class=\"chips\">"+chips(req,rc)+"</div></div>";
  if(hS) html+="<div class=\"board-row\"><span class=\"board-label lser\">SER</span><div class=\"chips\">"+chips(ser,sc)+"</div></div>";
  if(hD) html+="<div class=\"board-row\"><span class=\"board-label ldev\">DEV</span><div class=\"chips\">"+chips(dev,devCls)+"</div></div>";
  html+="</div>";
  return html;
}}
function sel(i){{
